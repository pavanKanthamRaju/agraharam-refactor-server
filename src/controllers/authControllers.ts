...
import { CreateUserPayload, GoogleUserInfo, UserCredentials } from '../types/index.js';
...
const login = async (req: Request, res: Response): Promise<void> => {
    const { identifier, password }: UserCredentials = req.body;
    ...
    if (!user) throw new AppError("invalid credentials Please provide valid email", 401);
...
const googleLogin = async (req: Request, res: Response): Promise<void> => {
    const { userInfo }: { userInfo: GoogleUserInfo } = req.body;
